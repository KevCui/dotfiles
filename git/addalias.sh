...
git config --global alias.d      "! $GIT_DIFF_NAME | fzf -0 -m --ansi --preview-window=right:80:wrap --preview 'git diff -- {} | $DIFF_CMD' --bind $FZF_KEYBINDING --bind $FZF_PREVIEW_KEYBINDING --bind 'enter:abort+execute(git diff -- {1} | $DIFF_CMD)' --cycle"

# show staged changes, enter to commit
git config --global alias.ds     "! $GIT_DIFF_NAME_CACHED | fzf -0 -m --ansi --preview-window=right:80:wrap --preview 'git diff --cached -- {} | $DIFF_CMD' --bind $FZF_KEYBINDING --bind $FZF_PREVIEW_KEYBINDING --bind 'enter:abort+execute(git commit)' --cycle"

# f <file>: show file change history with fzf
git config --global alias.f      '! sh -c "git log --follow --pretty=format:'\''%h %ad %s%d'\'' --date=short $1 | fzf +s -0 -m --preview-window=wrap --preview '\'''"$GIT_DIFF_TREE"' | '"$DIFF_CMD"''\'' --bind '"$FZF_KEYBINDING"' --bind '"$FZF_PREVIEW_KEYBINDING"' --bind '\''enter:abort+execute('"$GIT_DIFF_TREE"' | '"$DIFF_CMD"')'\'' --cycle" -'
...
# list chanege details of commit with fzf
git config --global alias.l      "! git log --pretty=format:'%h %ad %s%d' --date=short | fzf +s -0 -m --preview-window=wrap --preview '$GIT_DIFF_TREE | $DIFF_CMD' --bind $FZF_KEYBINDING --bind $FZF_PREVIEW_KEYBINDING --bind 'enter:abort+execute($GIT_DIFF_TREE | $DIFF_CMD)' --cycle"

# list summary of commit with fzf
git config --global alias.ls     "! git log --pretty=format:'%h %ad %s%d' --date=short | fzf +s -0 -m --preview-window=wrap --preview 'git show --color --compact-summary {1}' --bind $FZF_KEYBINDING --bind $FZF_PREVIEW_KEYBINDING --bind 'enter:abort+execute(git show --compact-summary {1})' --cycle"
